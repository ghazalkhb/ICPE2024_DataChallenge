...
folder_path1 = '/home/morteza/Desktop/Alireza/Dataset/data/CallGraph/data/service/plot_result'
...
for csv_file in csv_files:
    ...
    # Print the number of classes and the number of participants in each class
    num_classes = len(set(partition.values()))
    print(f"Number of Classes: {num_classes}")

    for class_id in set(partition.values()):
        participants_in_class = df[df['community'] == class_id]['source'].unique()
        num_participants_in_class = len(participants_in_class)
        print(f"Class {class_id}: {num_participants_in_class} unique participants")
